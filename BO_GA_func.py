...
class MF_BO:
    ...
    def next_observation(self,
                         ...
                         size_pop: int = 100,
                         iter_pop: int = 50,
                         plot_perfomance=True,
                         plot_af=True,
                         ):
        """ """
        '''1. Calculate the acquisition function.
              Then, query the new observation.'''
        if any(self.af_name == name for name in ['DMES', 'MES', 'VFMES']):
            self.y_best_set = Opt_Random_Sample(model=self.model,
                                                input_dim=self.input_dim,
                                                Size=100,
                                                RNG=self.rng,
                                                maximize=self.maximize)

        af_l = ft.partial(self.af_compute[self.af_name], fidelity='LF')
        af_m = ft.partial(self.af_compute[self.af_name], fidelity='HF')

        dim = self.input_dim
        if lb is None:
            lb = np.tile(np.array([0]), dim)
        if ub is None:
            ub = np.tile(np.array([1]), dim)
        ...
        ga_m = GA(af_m,
                  ...
                  lb=lb,
                  ub=ub)
        x_m_best, af_m_best = ga_m.run()

        if af_m_best < af_l_best:
            self.next_fidelity = 'HF'

            self.next_x = x_m_best
            self.next_x_af_value = af_m_best
            self.next_y = self.f_h(x_m_best)
        elif af_m_best >= af_l_best:
            self.next_fidelity = 'LF'

            self.next_x = x_l_best
            self.next_x_af_value = af_l_best
            self.next_y = self.f_l(x_m_best)
        else:
    ...
    def dmes(self, x, fidelity):
        x = np.asarray(x).reshape(-1, self.input_dim)
        y_best_set = self.y_best_set
        mean = self.model.meanPredict(x)
        std = self.model.stdPredict(x)
        ...
        if fidelity == 'HF':
            ...
            std_l = self.model.stdLowPredict(x)

            _, af = dmes_compute(y_best_set=y_best_set,  # self.y_best_set
                                 mean_h=mean,
                                 std_h=std,
                                 std_l=std_l,
                                 rho=self.model.rho,
    ...
    def vfei(self, x, fidelity):
        ...
        if fidelity == 'HF':
            ...
            std = self.model.stdLowPredict(x)
            std = abs(std * self.model.rho)

        af = ei_compute(mean=mean,
                        std=std,
                        y_best=self.y_best,
                        x0=0,
                        maximize=self.maximize)
    ...
    def efi(self, x, fidelity):
        ...
        if fidelity == 'HF':
            ...
            _, af = efi_compute(y_best=self.y_best,
                                mean_h=mean,
                                std_h=std,
                                std_l=std_l,
                                rho=self.model.rho,
    ...
    def mes(self, x, fidelity, n_sample: int = 100):
        x = np.asarray(x).reshape(-1, self.input_dim)
        if fidelity == 'HF':
            y_best_set = self.y_best_set
            mean = self.model.meanPredict(x)
    ...
    def vfmes(self, x, fidelity, n_sample: int = 100):
        pass
        return
