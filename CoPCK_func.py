...
class CoPCK():
    ...
    def fitModel(self, optMethod='Nelder-Mead'):
        '1.建立低精度模型'
        self.model_l = PCK(self.X_l, self.Y_l, dim=self.dim)
        ...
        X = np.vstack((self.X_l, self.X_h))
        yd_l = np.zeros(self.Y_l.shape)
        yd_h = self.Y_h - self.meanLowPredict(self.X_h) * rho
        yd = np.vstack((yd_l, yd_h))
        self.PCE = PCE(X, yd)
        self.mu = np.vstack((self.meanLowPredict(self.X_l), rho * self.meanLowPredict(self.X_h))) + self.PCE(X)
    ...
    def meanPredict(self, x):
        ...
        mean = self.rho * self.meanLowPredict(x) + self.PCE(x) + c_xX @ self.alpha
        return mean
    ...
    def varPredict(self, x, cov=False):
        ...
        if cov:
            Rh_xx = Rcompute(x, x, self.theta_P_h, self.Rtype_h)
            Rl_xx = Rcompute(x, x, self.model_l.theta_P, self.Rtype_l)
            C_xx = rho ** 2 * s2_l * Rl_xx + s2_h * Rh_xx + self.eps * np.eye(len(x))
        else:
            C_xx = (rho ** 2 * s2_l + s2_h + self.eps) * np.eye(len(x))

        Cov_xx = C_xx - c_xX @ self.invC @ c_xX.T
        # one = np.ones((n_h + n_l, 1))
        # Cov_xx = Cov_xx + (1 - one.T @ self.invC @ one)/ (one.T @ self.invC @ one)
