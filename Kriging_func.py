...
class Kriging():
    ...
    def likelihood(self, theta_P):
        ...
        try:
            ...
            one = np.ones((len(self.Y), 1))
            self.mu = (one.T @ self.invR @ self.Y) / (one.T @ self.invR @ one)

            ydelta = self.Y - self.mu
            self.sigma2 = (ydelta.T) @ self.invR @ ydelta / len(self.X)
            ...
            NLML = np.log(detR) + len(self.X) * np.log(self.sigma2)
        except:
            NLML = 1e4
    ...
    def varPredict(self, x, cov=False):
        ...
        if cov:
            R_xx = Rcompute(x, x, self.theta_P, self.Rtype) + self.eps * np.eye(len(x))
